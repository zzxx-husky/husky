...
class SyncShuffleCombiner : public ShuffleCombinerBase<MsgT, KeyT> {
   public:
    ...
    SyncShuffleCombiner(zmq::context_t* zmq_context) : zmq_context_(zmq_context) {}
   ...
   protected:
    void init_shuffle_itc() {
        ...
        sub_sock_->bind("inproc://sync-shuffle-combine-"+std::to_string(this->channel_id_)+"-"+std::to_string(this->local_id_));
        for(int tid : worker_info_.get_local_tids()) {
            auto push_sock = std::make_unique<zmq::socket_t>(*zmq_context_, ZMQ_PUSH);
            push_sock->connect("inproc://sync-shuffle-combine-"+std::to_string(this->channel_id_)+"-"+std::to_string(tid));
            if(push_sock_list_.size() < tid+1) push_sock_list_.resize(tid+1);
